...
const census = require('./census-handler')
const { MONGO } = require('../constants')
const { DATABASES, COLLECTIONS } = MONGO;
...
const methods = {
    ...
    getLoadedTradeZoneStats : async (pid, range) => {
        let res = await mongo.getRecord(DATABASES.LARNDMARK, 'tradezone_stats', {'pid': pid, 'range': range})
        return res
    },
...
getZoneStatsTracts : async (bounds) => {
    ...
    getZoneStatsZip : async (bounds) => {
        ...
        let statPromises = []
        for (let i=0; i < bounds.length; i++) {
            statPromises.push(census.getZipData(bounds[i][0]['state'], bounds[i][0]['zip-code-tabulation-area'], 'all'))
            statPromises.push(census.getZipData(bounds[i][0]['state'], bounds[i][0]['zip-code-tabulation-area'], 'age'))
        }
