...
let methods = {
    ...
    createTradezoneStats(pid, stats) {
        return new Promise(async (resolve, reject) => {
            let results = await mongo.writeOneToDB('locations', 'tradezone_stats', {'pid': pid, 'stats': stats, date: new Date().toString()})
            resolve({'res' :results, 'new tz stats': {'pid': pid, date: new Date().toString()}})
        })
    }
}
