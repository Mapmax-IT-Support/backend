...
let methods = {
    ...
    getListingByUserId : async function(user_id) {
        let res =  await mongo.getRecord('locations', 'listings', { "user_id" : String(user_id) })
        console.log('listing_from_user_id', res, user_id)
        return res
    },
    ...
    // delete listing
    deleteListing: async function(listinId) {
        let res = await mongo.deleteRecord('locations','listings', {listingId : listingId})
    ...
    getNearby : async function (zip, distance) {
        ...
        let nearbyListings = listings.filter((listing) => {
            console.log("DISTANCE", coords, listing.location.coords, calcCrow(coords.lat, coords.lng, listing.location.coords.lat, listing.location.coords.lng))
            return calcCrow(coords.lat, coords.lng, listing.location.coords.lat, listing.location.coords.lng) < distance
        })

        console.log("RES", nearbyListings.length)
