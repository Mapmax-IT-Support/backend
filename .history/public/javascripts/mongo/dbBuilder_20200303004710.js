...
//        console.log('ALL_BLOCKS', blocks)
        ...
        let stat_promises = []
        for (let i=2; i < 3; i++) {
            blocks[i].forEach(async e => {
                console.log(e.state, e.county, e.tract, e['block-group'])
           //     stat_promises.push(census.getAllBlockData(e.state, e.county, e.tract, e['block-group']))
            console.log(await getBlockAllStats(e.state, e.county, e.tract, e['block-group'])
            })
        }
