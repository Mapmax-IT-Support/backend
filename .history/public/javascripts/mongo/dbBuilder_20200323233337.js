...
const methods = {
    ...
    buildGeoJson : async () => {
        let data = require("../../gson/block-groups/block-group-ny.json")

        let res = await mongo.writeMany('gson', 'block-group-ny', data.features)
...
//        console.log('ALL_BLOCKS', blocks)
        ...
        for (let i=0; i < 100; i++) {
            blocks[i].forEach(e => {
                ...
                stat_promises.push(citysdk.getBlockAllStats(e.state, e.county, e.tract, e['block-group']))
            })
        }
