  ...
  //      let res = await mongo.deleteRecord('tract_stats', '34', {"social.summary.population": 0})
    ...
    buildBlockStats : async (state, county) => {
        ...
        for (let i=0; i < blocks; i++) {
            console.log(blocks[i].state, blocks[i].county, blocks[i].tract, blocks[i]['block-group'])
            stat_promises.push(getBlockAllStats(blocks[i].state, blocks[i].county, blocks[i].tract, blocks[i]['block-group']))
        }
...
const getBlockAllStats = async (state, county, tract, block) => {
    let allData = await citysdk.getTractAllStats(state, county, tract, block)
    let ageData = await citysdk.getTractAgeData(state, county, tract, block)
 //   let mappedData = mapToHeaders(allData)
