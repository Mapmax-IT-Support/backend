...
const methods = {
  ...
  //      let res = await mongo.deleteRecord('tract_stats', '34', {"social.summary.population": 0})
    ...
    buildBlockStats : async (state, county) => {
        let tracts = await citysdk.getAllTracts(state, county) // get all tracts 
   //     console.log('ALL_TRACTS', tracts)

        // get block groups for each tract
        let block_promises = []
        for (let i=0; i < tracts.length; i++) {
            block_promises.push(citysdk.getAllBlocks(tracts[i].state, tracts[i].county, tracts[i].tract))
        }

        let blocks = await Promise.all(block_promises)
//        console.log('ALL_BLOCKS', blocks)

        // get stats for each block
        let stat_promises = []
        for (let i=0; i < 1; i++) {
            blocks[i].forEach(e => {
                console.log(e.state, e.county, e.tract, e['block-group'])
                stat_promises.push(getBlockAllStats(e.state, e.county, e.tract, e['block-group']))
            })
            
        }

        let stats = await Promise.all(stat_promises)
        console.log('ALL_STATS', stats)
...
const getBlockAllStats = async (state, county, tract, block) => {
    let allData = await citysdk.getTractAllStats(state, county, tract, block).catch(e => console.error(e))
    let ageData = await citysdk.getTractAgeData(state, county, tract, block).catch(e => console.error(e))
    console.log('TEST', allData, ageData)
 //   let mappedData = mapToHeaders(allData)
